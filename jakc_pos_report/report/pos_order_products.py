...
class pos_details(report_sxw.rml_parse):
    ...
    def _pos_sales_details(self,product_id,form):
        ...
        query  = """SELECT 
              ...
              COALESCE(mrp_bom_line.product_qty * pos_order_line.qty,0) as total_qty
            FROM pos_order
    ...
    def _get_per_product_bom_summary(self, product_id, form):
        ...
        query = """SELECT  
                  product_bom_line.name_template,
                  COALESCE(sum(mrp_bom_line.product_qty * pos_order_line.qty),0) as total_qty,
                  product_uom_bom_line.name as product_uom_name
                FROM pos_order
                LEFT JOIN pos_order_line ON pos_order.id = pos_order_line.order_id 
                    LEFT JOIN product_product ON pos_order_line.product_id = product_product.id
                  ...
                  pos_order.id IN {} AND pos_order_line.product_id IN {}
                GROUP BY
                  product_bom_line.name_template, product_uom_bom_line.name
                ORDER BY product_bom_line.name_template;""".format(tuple(pos_ids),tuple(product_ids))
